...
for index, dado in enumerate(dados_cama_elastica):
    a, b, c,d = round(dado[0],1)
    color_i = colors[str(index+1)]
    pertinencia = [Trapezoidal(round(x, 1), a, b, c, d) for x in altura_de_saltos_em_cama_elastica]

    ax_cama_elastica.plot(altura_de_saltos_em_cama_elastica, pertinencia, label=f'Pertinência {a}, {b}, {c}, {d} : {dado[1]}', color=color_i, linewidth = 2)
...
for index, dado in enumerate(dados_basquete):
    a, b, c,d = round(dado[0],1)
    color_i = colors[str(index+1)]
    pertinencia = [Trapezoidal(round(x, 1), a, b, c, d) for x in altura_de_saltos_para_enterrada_no_basquete]

    ax_basquete.plot(altura_de_saltos_para_enterrada_no_basquete, pertinencia, label=f'Pertinência {a}, {b}, {c}, {d} : {dado[1]}', color=color_i, linewidth = 2)
